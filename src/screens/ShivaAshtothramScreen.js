...
export default function shivaAshtothramScreen() {
    ...
    const heading = language === 'ta' ? 'சிவ அஷ்டோத்திரம்' : 'shiva Ashtothram';
    ...
    useEffect(() => {
        async function fetchData() {
            setLoading(true);
            try {
                const docRef = doc(db, 'shivaAshtothram', 'content');
                ...
                {showGeneralInfo && (
                    <View style={[styles.accordion, { backgroundColor: currentTheme.accent, width: isWide ? 600 : '100%', alignSelf: 'center', marginBottom: 16, marginTop: 8 }]}>
                        ...
                        <Text style={{ fontSize, color: currentTheme.text, lineHeight: 22, fontWeight: bold ? 'bold' : 'normal', textAlign: 'left', alignSelf: 'stretch' }}>
                            {language === 'ta'
                                ? 'சிவ அஷ்டோத்திரம் என்பது சிவ பக்தர்களால் விரும்பி பாடப்படும் பாடல் தொகுப்பு. (இங்கு விவரங்களை மாற்றவும்)'
                                : 'shiva Ashtothram is a popular devotional hymn dedicated to Lord shiva. (Update this info as needed)'}
                        </Text>
                    </View>
                )}
                {filteredPoem && (
