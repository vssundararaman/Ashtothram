...
import shivaImg from '../assets/images/shiva.png'; // TODO: Replace with Lakshmi image
import MaterialIcons from 'react-native-vector-icons/MaterialIcons';
import PinchZoomView from '../../PinchZoomView';
import { db } from '../services/firebase';
import { doc, getDoc } from 'firebase/firestore';

export default function LakshmiAshtothramScreen() {
    ...
    const heading = language === 'ta' ? 'மகா லட்சுமி அஷ்டோத்திரம்' : 'Lakshmi Ashtothram';
    ...
    useEffect(() => {
        async function fetchData() {
            setLoading(true);
            try {
                const docRef = doc(db, 'lakshmiAshtothram', 'content');
                ...
                {showGeneralInfo && (
                    <View style={[styles.accordion, { backgroundColor: currentTheme.accent, width: isWide ? 600 : '100%', alignSelf: 'center', marginBottom: 16, marginTop: 8 }]}> 
                        ...
                        <Text style={{ fontSize, color: currentTheme.text, lineHeight: 22, fontWeight: bold ? 'bold' : 'normal', textAlign: 'left', alignSelf: 'stretch' }}>
                            {language === 'ta'
                                ? 'மகா லட்சுமி அஷ்டோத்திரம் என்பது லட்சுமி பக்தர்களால் விரும்பி பாடப்படும் பாடல் தொகுப்பு.'
                                : 'Lakshmi Ashtothram is a popular devotional hymn dedicated to Goddess Lakshmi.'}
                        </Text>
                    </View>
                )}
                {filteredPoem && (
